...
while IFS= read -r line; do
	...
	sbatch \
                ...
                --mem=$MEM \
                --cpus-per-task=$CORES \
                --job-name=cut_map_$acc \
                --output=./log_files/cut_map_$acc.log \
                --error=./log_files/cut_map_$acc.error.log \
                --partition=single \
                --export=CORES=$CORES,WORKTMP=$WORKTMP,input_fastq=$input_fastq,REF=$REF,acc=$acc \
                ./script_trim_qc_map.sh

done < $WORKTMP/$SAMPLE_FILE



#USAGE:  # ./submit_trim_qc_map.sh <CORES> <MEM> <TIME> <WORKTMP> <input_fastq> <REF> <SAMPLE_FILE>
